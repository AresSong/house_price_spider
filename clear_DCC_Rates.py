...
# DCC_rates = pd.read_sql(r"select * from [load_rates_20170214]  WHERE [index] between 1 and 20",engine)
DCC_rates = pd.read_sql(r"""SELECT DISTINCT STREET+' ' + [suburb] + ' ' + city as property_address
,[url] FROM  [housing].[dbo].[load_listings]
WHERE STREET IS NOT NULL AND STREET <> ''""",engine)
...
DCC_rates_unified_address.to_sql("Listings_unified_address",engine,if_exists = "append")